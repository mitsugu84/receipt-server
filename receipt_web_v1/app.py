...

{
  ...
  "recipient": "宛名・患者名・顧客名 または 空文字",
  "issuer": "発行元店舗名・会社名・病院名・医療機関名 または 不明",
...
【最重要】
- recipient は宛名・患者名・顧客名・利用者名です。
- issuer は領収書を発行した店舗名・会社名・病院名・医療機関名です。
- 「○○様」は recipient です。issuer ではありません。
- 人名を issuer にしてはいけません。
- 最終的な店名には issuer を使うため、issuer を慎重に抽出してください。
- 読めない情報は推測せず「不明」にしてください。

【医療領収書の特別ルール】
- 「医療機関：」「薬局名：」「発行元：」の後ろにある名称を issuer としてください。
- 「○○様」と書かれた大きい文字は recipient です。
- 医療機関名と患者名が両方ある場合、患者名を issuer にしてはいけません。
- 金額は「負担金」「領収額合計」を優先してください。
- 日付は「調剤日」「発行日」「診療日」を優先してください。
- 病院、内科、歯科、薬局、調剤の領収書は category を 医療費 にしてください。

【日付】
- 「発行日」「領収日」「取引日」「調剤日」「購入日」を優先してください。
- 有効期限、ポイント期限、キャンペーン期限、登録番号、電話番号は日付として使わないでください。
- 複数の日付がある場合は、実際の購入日・利用日・診療日を採用してください。

【issuer】
- 店舗名、会社名、病院名、薬局名、屋号を優先してください。
- ロゴ、住所、電話番号の近くにある名称も発行元候補として確認してください。
- 宛名、患者名、顧客名、利用者名、人名は issuer にしないでください。
- 漢字を勝手に似た漢字へ補正しないでください。
- 自信がない場合は memo に「発行元要確認」と書いてください。

【金額】
- 支払合計、合計、税込合計、現計、クレジット支払額、領収額合計を優先してください。
- 小計、税額、ポイント、預り金、お釣りは合計金額にしないでください。
- 不明なら0にしてください。

【区分】
- コンビニ、スーパー、飲食店 → 食費
- 電車、バス、高速代、駐車場、タクシー → 交通費
- ホームセンター、WORKMAN、工具購入、作業用品 → 工具費
- 文房具、日用品、雑貨 → 消耗品費
- 携帯電話、ネット回線、通信料金 → 通信費
- 接待、会食、贈答 → 交際費
- 病院、クリニック、内科、歯科、薬局、調剤 → 医療費
- 判断できない場合のみ その他

【出力ルール】
- JSONのみ返してください。
- 推測で埋めないでください。
- 不確かな情報は memo に「要確認」と記載してください。
"""
    ...
    issuer = str(data.get("issuer", "不明") or "不明")
    recipient = str(data.get("recipient", "") or "")
    memo = str(data.get("memo", "") or "")

    if recipient and recipient == issuer:
        issuer = "不明"
        memo = (memo + " 宛名と発行元が同一のため要確認").strip()
